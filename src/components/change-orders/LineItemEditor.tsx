...
/**
 * LineItemEditor Component for Change Orders
 *
 * Reusable line item editor for change orders.
 * Unlike purchase orders, supports negative amounts for credits/scope reductions.
 *
...
export function LineItemEditor({
  ...
  const handleUpdate = useCallback(
    (id: string, field: keyof Omit<ChangeOrderLineItem, 'id' | 'total'>, value: string | number) => {
      onChange(
        items.map((item) => {
  ...
  const hasNegativeItems = items.some((item) => item.total < 0)
  const hasPositiveItems = items.some((item) => item.total > 0)
      ...
      {items.map((item, index) => (
        ...
        >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            {/* Unit Price (can be negative for credits) */}
            <div>
              <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                Stueckpreis
              </label>
            ...
            {/* Total (calculated, readonly, shows +/- indicator) */}
            <div>
              <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">
                Total
              </label>
              <div
                className={`w-full h-10 px-3 flex items-center rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm font-medium ${
                  item.total < 0
              ...
              >
                {item.total < 0 ? '' : '+'}{formatCHF(item.total)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              {hasPositiveItems && hasNegativeItems && (
                <p className="text-xs text-gray-500">
                  (Enthält Ergänzungen und Gutschriften)
                </p>
              )}
            </div>
            <div
