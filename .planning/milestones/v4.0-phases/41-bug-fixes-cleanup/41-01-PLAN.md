...
3. Pass `orgId` to `fetchHeatmapData(buildingId, orgId)`
  ...
  <verify>
Run `npx tsc --noEmit --pretty` — no type errors. Grep for `createPublicClient` in cached-queries.ts — zero matches. Grep for `set_org_context` in cached-queries.ts — should match inside createOrgScopedClient. Grep for `orgId` in heatmap-queries.ts and dashboard-queries.ts — should match in function signatures and calls. Grep for `getCachedActiveProjectCount` in dashboard-queries.ts — confirm two-argument form `(unitIds, orgId)` not single-argument.
  </verify>
  <done>
