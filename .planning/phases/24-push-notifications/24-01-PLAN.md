...
<objective>
Create the notification database schema, TypeScript types, service worker, and push subscription management.

Purpose: Foundation layer for the entire push notification system. All other notification plans depend on these artifacts.
Output: Migration with 4 tables + pg_cron jobs, types file, service worker, PushContext, subscribe/unsubscribe APIs, next.config update.
</objective>
...
2. `send_daily_digests()` - Placeholder function that logs digest candidates (full implementation in Plan 24-02 when send logic exists). For now, just SELECT user_id, digest_time FROM notification_preferences WHERE digest_enabled = TRUE. Body: RAISE LOG with user_id and unread count.
...
<task type="auto">
  <name>Task 3: Service worker, PushContext, subscribe API, next.config</name>
  <files>
    public/sw.js
    next.config.ts
    src/contexts/PushContext.tsx
...
Verify: next.config.ts has headers() with sw.js cache control.
  ...
  <done>Service worker handles push events and notification clicks. PushContext registers SW and manages subscriptions. Subscribe/unsubscribe APIs store subscriptions in database. VAPID public key exposed to client only.</done>
</task>
