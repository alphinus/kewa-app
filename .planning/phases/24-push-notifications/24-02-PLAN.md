...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/lib/notifications/send.ts"
      provides: "Core send function with preference checks, quiet hours, urgency bypass, push delivery"
...
<objective>
Build notification preferences management and the core send notification function with quiet hours, urgency bypass, and push delivery.

Purpose: Users control which notifications they receive and when. The send function is the central dispatch used by all event triggers in Plan 24-03.
Output: Preferences API + UI page, core send function with web-push integration.
</objective>
...
<task type="auto">
  <name>Task 2: Send notification function and preferences UI</name>
  <files>
    src/lib/notifications/send.ts
    src/app/dashboard/settings/benachrichtigungen/page.tsx
  </files>
...
Verify: /dashboard/settings/benachrichtigungen page renders with preference toggles.
  ...
  <done>Send function dispatches notifications respecting preferences, quiet hours, urgency bypass, and digest mode. Preferences page lets users configure all notification settings with role-based type visibility.</done>
</task>
...
8. Settings page shows role-appropriate notification types
</verification>
