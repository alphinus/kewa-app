...
- Inspection with overall_result='passed_with_conditions' → condition becomes 'partial'
  ...
  <verify>
Run `npx tsc --noEmit` to verify types. Run `npm run build` to verify all files compile.
  </verify>
  <done>
Types include acknowledgment fields. Queries fetch portal data. Room condition trigger documented for testing.
  </done>
</task>
