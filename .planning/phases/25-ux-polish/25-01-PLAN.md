...
- The `[id]` in the URL is a building ID (consistent with existing child routes like `liegenschaft/[id]/einheit/[unitId]/raum/[roomId]`).

- Approach:
  1. Fetch building info: `GET /api/buildings/{id}` to get building name and property_id
  2. Reuse `<DeliveryList propertyId={building.property_id} />` without `onLinkInvoice` prop (component already hides the link button when `onLinkInvoice` is not passed)
  3. No new API endpoints needed -- existing deliveries API supports `property_id` filter
  </action>
  <verify>
1. `npx tsc --noEmit` passes without errors
