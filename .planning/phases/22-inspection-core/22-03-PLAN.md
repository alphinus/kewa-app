...
- `createFollowUpTask(defect: InspectionDefect, inspection: Inspection, assigneeId: string)`: Creates a task in tasks table linked to the inspection's work order. First, look up the work order to get its project context. Fields: title = "Mangel: {defect.title}", description = defect.description + " (Schweregrad: {severity})", project_id = inspection.project_id (or from work order), work_order_id = inspection.work_order_id, status = 'open', priority = severity-based ('schwer' -> 'urgent', 'mittel' -> 'high', 'gering' -> 'normal'), assigned_to = assigneeId. If work_orders table has a parent task reference, use it for subtask linking; otherwise create as a standalone task associated with the work order. Returns created task. Updates defect: set action='task_created', linked_task_id=task.id. Check actual tasks table schema before implementation — the executor should `grep -n "CREATE TABLE tasks" supabase/migrations/*.sql` to determine exact column names for work order association and subtask linking.
