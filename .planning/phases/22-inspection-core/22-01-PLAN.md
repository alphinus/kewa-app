...
<task type="auto">
  ...
  <action>
Install react-signature-canvas (needed in Plan 22-03, but install now to avoid dependency issues):
...
**Storage bucket:** Add SQL comment at the top noting that an `inspections` storage bucket must be created in Supabase dashboard (path pattern: inspections/{inspection_id}/items/, inspections/{inspection_id}/defects/, inspections/{inspection_id}/signature.png).
  </action>
  <verify>
...
Check: `npm ls react-signature-canvas` confirms package installed.
  ...
  <done>All three tables exist with correct columns, enums, triggers, and indexes. react-signature-canvas is installed.</done>
</task>
