...
Also update the `/api/` matcher to exclude portal auth routes from operator session checking. Portal API routes under `/api/portal/` (non-auth) use portal session, not operator session.
